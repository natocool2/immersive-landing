...
const GlassDock: React.FC<{ icons: DockIcon[]; href?: string; activeIndex?: number }> = ({
  ...
  <GlassEffect
    href={href}
    className="rounded-3xl p-2 md:p-3 lg:p-3"
  >
    <div className="flex items-center justify-center gap-1 md:gap-2 lg:gap-2 rounded-3xl p-2 md:p-3 lg:p-3 py-0 px-0.5">
      {icons.map((icon, index) => (
        <div key={index} className="relative">
          <img
            src={icon.src}
            alt={icon.alt}
            className={`w-12 h-12 md:w-16 md:h-16 lg:w-16 lg:h-16 transition-all duration-700 cursor-pointer -translate-y-px ${
              activeIndex === index 
                ? 'scale-110' 
                : 'hover:scale-110'
            }`}
