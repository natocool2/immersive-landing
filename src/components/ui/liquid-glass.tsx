...
const GlassDock: React.FC<{ icons: DockIcon[]; href?: string; activeIndex?: number }> = ({
  ...
  <GlassEffect
    href={href}
    className="rounded-3xl p-2 md:p-4"
  >
    <div className="flex items-center justify-center gap-1 md:gap-3 rounded-3xl p-1 md:p-2 py-0 px-0.5">
      {icons.map((icon, index) => (
        <div key={index} className="relative">
          <img
            src={icon.src}
            alt={icon.alt}
            className={`w-12 h-12 md:w-16 md:h-16 transition-all duration-700 cursor-pointer -translate-y-px ${
              activeIndex === index 
                ? 'scale-110' 
                : 'hover:scale-110'
            }`}
          ...
          />
          {activeIndex === index && (
            <div className="absolute bottom-[-6px] md:bottom-[-8px] left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 md:w-2 md:h-2 bg-black/60 rounded-full transition-all duration-300" />
          )}
        </div>
      ))}
    </div>
...
const GlassInput: React.FC<{ placeholder?: string }> = ({
  ...
  <GlassEffect className="rounded-3xl px-6 py-4 overflow-hidden w-full max-w-2xl">
    <input
      ...
      className="w-full bg-transparent text-white placeholder-white/70 text-xl outline-none transition-all duration-700"
      style={{
        transitionTimingFunction: "cubic-bezier(0.175, 0.885, 0.32, 2.2)",
      }}
    />
...
export { GlassEffect, GlassDock, GlassButton, GlassInput, GlassFilter };